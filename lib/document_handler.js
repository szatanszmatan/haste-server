...
DocumentHandler.prototype.handleListing = function (request, response) {
  this.store.listing(function (ret) {
      response.writeHead(200, {'content-type': 'text/html; charset=utf-8'});
      var hostUrl = request.headers["host"];
      response.write("<html><h3>Latest used keys:</h3>");
      ret.forEach(function (key) {
        var line = "<a href='http://" + hostUrl + "/" + key[1] + "'>" + key[1] + "</a> - " + key[0] + " s. ago<br />";
        response.write(line);
      });
      response.end("</html>");
    });
